...
document.getElementsByClassName("form__submit")[0].addEventListener("click", function () {
    ...
    var captcha = document.getElementById("ranstring").value;
    ...
    ajax("post", "/login", JSON.stringify({ id: id, password: password, captcha: captcha, cookie: cookie }), function (responseText) {
        if (responseText.error == "err") {
            ...
            if (result.err) {
                ...
                var loginResult = result.text;
                console.log(result);
                if (loginResult.startsWith("登录成功")) {
                    alert("登录", loginResult, function () {
                        if (result.isVerify) window.location.href = "index?id=" + id + "&token=" + result.token;else {
                            window.location.href = "telVerify?id=" + id + "&token=" + result.token;
                        }
                    }, { type: 'success' });
                } else {
                    alert("登录", loginResult, function () {}, { type: 'error' });
                }
